...
while True:
    ...
    p = np.random.randint(4)  # Losowanie przykładu
    input_p = inputs[p].reshape(1, -1)
    target_p = targets[p].reshape(1, -1)
    
    # Forward pass
    hidden_input = np.dot(input_p, w_old.T)
    ...
    DE_w = np.dot(hidden_error[:, :-1].T, input_p)
...
print("Testowanie bramki XOR SGD:")
for i in range(4):
